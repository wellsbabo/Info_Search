...
router.post('/search',(req,res,next) => {
    ...
    afterStrData = req.body.searchData.split('\n');
/*     var finalData = []
    afterStrData.forEach(element => {
        console.log(element)
        if(element!=''){
            val = new RegExp(element)
            finalData.push(val)
        }
    }); */
    ...
    dataModel.find({$or:[
        {ipaddr:{$in:afterStrData}},
        {gname:{$in:afterStrData}},
        {etc:{$in:afterStrData}}
        //{date:{$in:finalData}}
    ]},function(err, docs){
        if(err){
...
router.post('/contentDetail',(req,res,next) => {

    ID = req.body.id

    dataModel.findOne({_id:ID},function(err, docs){
        if(err){
            res.json(err)
        }
        res.json(docs)
